...
class InvoiceGeneratorService extends MedusaService({
      ...
      header: {
        ...
        columns: [
          /** Company Logo */
          {
            width: '*',
            stack: [
              ...(config.company_logo ? await this.safeLogo(config.company_logo) : []),
              {
  ...
  private async safeLogo(url: string): Promise<any[]> {
  try {
    return [
      {
        image: await this.imageUrlToBase64(url),
        width: 80,
        height: 40,
        fit: [80, 40],
        margin: [0, 0, 0, 10],
    ...
    console.warn(`Invoice logo could not be loaded: ${url}`)
    return []
  }
}
